...
const storyTemplates = [
  ...
  (f1, f2) => `const friendship = ${f1}.isBestFriendWith(${f2}); // 우정이 +999 증가했습니다. ✨`,
  ...
  (f1, f2) => `try { ${f1}님이 ${f2}님에게 롤 듀오를 신청합니다. } catch (error) { "거절당했습니다." }`,
  (f1, f2) => `// ${f1}님이 ${f2}님의 의자 높이를 몰래 낮춰놨습니다. 🪑`,
  (f1, f2) => `for (let i = 0; i < 5; i++) { console.log("${f1}님이 ${f2}님을 괴롭힙니다. 😂"); }`
];
